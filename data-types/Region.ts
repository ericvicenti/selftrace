...
class Region {
  ...
  contains(lat: number, lng: number): boolean {
    if (lat > this.latitude + this.latitudeDelta || lat < this.latitude - this.latitudeDelta) {
      return false;
    }

    if (lng > this.longitude + this.longitudeDelta || lng < this.longitude - this.longitudeDelta) {
      return false;
    }
