...
export default class Color {
  ...
  private static validateRGB(red: number, green: number, blue: number): void {
    function validateField(field: 'red' | 'green' | 'blue', value: number) {
      if (!Number.isSafeInteger(value) || value < 0 || value > 255)
        throw new Error(`${field} value must be a safe integer between 0 and 255.`);
    }
  ...
  shade(percent: number): string {
    ...
    return Color.makeRGBString(scale(this.red), scale(this.green), scale(this.blue));
  }
  ...
  private static makeRGBString(red: number, green: number, blue: number): string {
